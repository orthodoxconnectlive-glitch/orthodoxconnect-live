...
export const AudioPlayer: React.FC<AudioPlayerProps> = ({
  ...
  return (
    <div
      id={elementMediaId}
      className={`rounded-2xl p-3.5 bg-gradient-to-r from-[#2c1d11] via-(--chip-dark) to-[#2c1d11] border border-(--ln-gold)/40 shadow-lg text-[#f5ebd9] ${className}`}
    >
      ...
      {hasError ? (
        <div className="flex items-center justify-between gap-3 text-xs text-(--chip-light)/80 font-serif">
          ...
          <button
            ...
            className="px-2.5 py-1 rounded-lg bg-(--chip-dark) hover:bg-(--ac-gold) text-(--ac-gold-tx) hover:text-(--tx-ink) text-[10px] font-serif font-bold uppercase transition-colors border border-(--ln-gold)/30 flex items-center gap-1 cursor-pointer"
          >
        ...
        <div className="flex items-center gap-3.5">
          ...
          <button
            ...
            className={`w-12 h-12 rounded-full flex items-center justify-center transition-all transform hover:scale-105 active:scale-95 shadow-md cursor-pointer shrink-0 border ${
              isPlaying
                ? 'bg-gradient-to-br from-(--ac-gold) to-(--ac-bronze-dk) text-(--tx-ink) border-(--ln-light)'
                : 'bg-gradient-to-br from-[#8f6e30] to-[#5a4220] hover:from-(--ac-gold) hover:to-(--ac-bronze-dk) text-[#f5ebd9] border-(--ln-gold)/60'
            }`}
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between gap-2 mb-1">
              <div className="flex items-center gap-1.5 min-w-0">
                <Music className={`w-3.5 h-3.5 text-(--ac-gold-tx) shrink-0 ${isPlaying ? 'animate-bounce' : ''}`} />
                <span className="text-xs font-serif font-bold text-[#f5ebd9] truncate">
                  {title}
                </span>
              </div>
              {authorName && (
                <span className="text-[10px] text-(--ac-gold-tx) font-serif uppercase tracking-wider shrink-0">
                  {authorName}
                </span>
              )}
            </div>
            ...
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-mono text-(--chip-light)/80 w-8 text-right">
              ...
              <input
                ...
                value={currentTime}
                onChange={handleSeek}
                className="flex-1 h-1.5 bg-[#1c130c] rounded-lg appearance-none cursor-pointer accent-(--ac-gold)"
                title="Seek audio track"
              />
              <span className="text-[10px] font-mono text-(--chip-light)/80 w-8">
          ...
          {/* Volume / Mute Control */}
          <div className="hidden sm:flex items-center gap-1.5 pl-2 border-l border-(--ln-gold)/20 shrink-0">
            <button
              type="button"
              onClick={handleToggleMute}
              className="p-1 text-(--chip-light)/80 hover:text-(--ac-gold-tx) transition-colors cursor-pointer"
            ...
            <input
              ...
              className="w-14 h-1 bg-[#1c130c] rounded-lg appearance-none cursor-pointer accent-(--ac-gold)"
              title="Adjust volume"
            />
          </div>
        </div>
