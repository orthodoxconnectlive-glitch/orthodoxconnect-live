...
export const GroupRoomsView: React.FC<GroupRoomsViewProps> = ({ onSelectUser, onOpenMessengerWithUser }) => {
  ...
  return (
    <div className="space-y-6">
      {/* Top Banner Header */}
      <div className="relative rounded-3xl bg-(--bg-card) dark:bg-[#1c1611] border-2 border-(--ln-gold) dark:border-[#8b6b4a] p-6 shadow-xl overflow-hidden">
        <div className="absolute -right-10 -bottom-10 w-48 h-48 bg-(--ac-gold)/10 rounded-full blur-2xl pointer-events-none" />

        <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3.5">
            <div className="w-12 h-12 rounded-2xl bg-(--chip-dark) text-(--ac-gold-tx) font-bold text-xl flex items-center justify-center border-2 border-(--ln-gold) shadow-md shrink-0">
              ☨
            </div>
            <div>
              <h2 className="font-serif-coptic font-bold text-xl sm:text-2xl text-(--tx-strong) dark:text-[#f5ebd9] uppercase tracking-wider">
                {t('myNetwork')}
              </h2>
          ...
          <div className="relative w-full sm:w-64">
            <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-(--tx-mute) dark:text-[#a89379]" />
            <input
              type="text"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              placeholder="Filter members or groups..."
              className="w-full pl-9 pr-3 py-2 text-xs font-serif rounded-2xl bg-(--bg-soft) dark:bg-[#282019] border border-(--ln-gold) text-(--tx-strong) dark:text-[#f5ebd9] placeholder-(--tx-mute)/70 focus:outline-none"
            />
          </div>
        </div>

        {/* View Switcher Tabs */}
        <div className="mt-6 pt-4 border-t border-(--ln-gold)/30 flex items-center gap-2 overflow-x-auto no-scrollbar font-serif text-xs">
          <button
            ...
            className={`px-4 py-2.5 rounded-2xl font-bold uppercase tracking-wider flex items-center gap-2 transition-all cursor-pointer ${
              activeTab === 'followed'
                ? 'bg-(--chip-dark) text-(--ac-gold-tx) border-2 border-(--ln-gold) shadow-md'
                : 'bg-(--bg-soft) dark:bg-[#282019] text-(--tx-mute) dark:text-[#a89379] hover:text-(--tx-strong) border border-(--ln-gold)/40'
            }`}
          >
            <Users className="w-4 h-4 text-(--ac-gold-tx)" />
            <span>{t('peopleIFollow')}</span>
            <span className="px-2 py-0.5 rounded-full bg-(--ac-gold) text-(--tx-strong) text-[10px] font-bold">
          ...
          >
            <Church className="w-4 h-4 text-(--ac-gold-tx)" />
      ...
      {activeTab === 'followed' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between font-serif text-xs px-1">
            <h3 className="font-serif-coptic font-bold uppercase tracking-wider text-(--tx-strong) dark:text-[#f5ebd9] flex items-center gap-2">
              <span>Parish Members You Follow</span>
              <span className="text-(--ac-bronze-tx)">({filteredFollowed.length})</span>
            </h3>
            <button
              onClick={() => setActiveTab('discover')}
              className="text-(--ac-bronze-tx) hover:underline font-bold uppercase tracking-wider text-[11px] flex items-center gap-1 cursor-pointer"
            >
          ...
          {filteredFollowed.length === 0 ? (
            <div className="p-8 text-center bg-(--bg-card) dark:bg-[#1c1611] rounded-3xl border-2 border-(--ln-gold) text-(--tx-mute) text-xs font-serif space-y-3">
              <p className="font-bold text-(--tx-strong) dark:text-[#f5ebd9]">
                You are not following any parish members yet.
              </p>
              <button
                onClick={() => setActiveTab('discover')}
                className="px-4 py-2 rounded-xl bg-(--ac-bronze) text-white font-bold uppercase tracking-wider cursor-pointer"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {filteredFollowed.map((member) => (
                <div
                  key={member.id}
                  className="bg-(--bg-card) dark:bg-[#1c1611] border-2 border-(--ln-gold) dark:border-[#8b6b4a] rounded-3xl p-5 shadow-lg flex flex-col justify-between space-y-4"
                >
                  <div className="flex items-start gap-3.5">
                    ...
                    >
                      <img
                        src={member.avatar}
                        alt={member.name}
                        className="w-14 h-14 rounded-2xl object-cover border-2 border-(--ln-gold)"
                      />
                      {member.isOnline && (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-1">
                        <h4
                          className="font-serif-coptic font-bold text-sm text-(--tx-strong) dark:text-[#f5ebd9] uppercase tracking-wider truncate cursor-pointer hover:underline hover:text-(--ac-gold-tx) transition-colors"
                          onClick={() =>
                            onSelectUser?.({
                              id: member.id,
                              name: member.name,
                        ...
                        <span className="px-2 py-0.5 rounded-full bg-(--bg-soft) dark:bg-[#282019] text-(--tx-strong) dark:text-[#f5ebd9] text-[9px] font-serif font-bold uppercase tracking-wider border border-(--ln-gold)">
                          Following
                        </span>
                      </div>

                      <p className="text-[11px] text-(--ac-bronze-tx) font-serif font-bold uppercase tracking-wider mt-0.5">
                        {member.title}
                      </p>

                      <p className="text-[10px] text-(--tx-mute) dark:text-[#a89379] font-serif flex items-center gap-1 mt-1">
                        <Church className="w-3 h-3 text-(--ac-gold-tx)" /> {member.parish}
                      </p>

                      <p className="text-xs text-(--tx-strong) dark:text-[#f5ebd9] font-serif leading-relaxed mt-2 line-clamp-2">
                        "{member.bio}"
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="pt-3 border-t border-(--ln-gold)/30 flex items-center justify-end gap-2 font-serif text-xs">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="px-3 py-1.5 rounded-xl bg-(--bg-soft) dark:bg-[#282019] text-(--tx-mute) dark:text-[#a89379] hover:bg-red-900/20 hover:text-red-600 border border-(--ln-gold) font-bold uppercase tracking-wider flex items-center gap-1 cursor-pointer transition-colors"
                      >
                        <UserCheck className="w-3.5 h-3.5 text-(--ac-bronze-tx)" />
                        <span>Unfollow</span>
                      </button>
                      ...
                      {onOpenMessengerWithUser && (
                        <button
                          ...
                          className="px-3 py-1.5 rounded-xl bg-(--chip-dark) dark:bg-[#282019] text-(--ac-gold-tx) hover:bg-(--ac-bronze) hover:text-white border border-(--ln-gold) font-serif font-bold uppercase tracking-wider flex items-center gap-1 cursor-pointer transition-all shadow-sm"
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between font-serif text-xs px-1">
            <h3 className="font-serif-coptic font-bold uppercase tracking-wider text-(--tx-strong) dark:text-[#f5ebd9] flex items-center gap-2">
              ...
              <span className="text-(--ac-bronze-tx)">({filteredJoinedGroups.length})</span>
            </h3>
            <button
              onClick={() => setActiveTab('discover')}
              className="text-(--ac-bronze-tx) hover:underline font-bold uppercase tracking-wider text-[11px] flex items-center gap-1 cursor-pointer"
                ...
                >
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      <div className="w-12 h-12 rounded-2xl bg-(--bg-soft) dark:bg-[#282019] border border-(--ln-gold) flex items-center justify-center text-2xl shadow-sm">
                        {group.icon}
                      </div>
                  ...
                  <div className="pt-3 border-t border-(--ln-gold)/30 flex items-center justify-between gap-2 font-serif text-xs">
                    <div>
                      <span className="text-[10px] text-(--ac-bronze-tx) font-bold uppercase tracking-wider block">
                        Host: {group.hostName}
                      </span>
                      <span className="text-[9px] text-(--tx-mute) block">{group.parish}</span>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="px-4 py-2 rounded-xl bg-(--ac-bronze) hover:bg-(--ac-bronze-dk) text-white font-bold uppercase tracking-wider flex items-center gap-1.5 shadow-md cursor-pointer text-[11px]"
                      >
      ...
      {activeTab === 'discover' && (
        <div className="space-y-8">
          ...
          <div className="space-y-4">
            <h3 className="font-serif-coptic font-bold text-sm text-(--tx-strong) dark:text-[#f5ebd9] uppercase tracking-wider flex items-center gap-2">
              <Church className="w-4 h-4 text-(--ac-gold-tx)" />
                  ...
                  >
                    <div>
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <span className="px-3 py-1 rounded-full bg-(--bg-soft) dark:bg-[#282019] border border-(--ln-gold) text-(--tx-strong) dark:text-[#f5ebd9] text-[10px] font-serif font-bold uppercase tracking-wider">
                      ...
                      <button
                        ...
                        className={`px-4 py-2 rounded-xl font-bold uppercase tracking-wider flex items-center gap-1.5 shadow-md transition-all cursor-pointer text-xs ${
                          isJoined
                            ...
                            : 'bg-(--ac-bronze) hover:bg-(--ac-bronze-dk) text-white'
                        }`}
                      >
                        {isJoined ? (
                          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {membersList.map((member) => {
                ...
                return (
                  <div
                    key={member.id}
                    className="bg-(--bg-card) dark:bg-[#1c1611] border-2 border-(--ln-gold) dark:border-[#8b6b4a] rounded-3xl p-4 shadow-lg flex items-center justify-between gap-3"
                  >
                    <div
                    ...
                    >
                      <img
                        src={member.avatar}
                        alt={member.name}
                        className="w-12 h-12 rounded-2xl object-cover border-2 border-(--ln-gold) shrink-0"
                      />
                      <div className="min-w-0">
                        <h4 className="font-serif-coptic font-bold text-xs text-(--tx-strong) dark:text-[#f5ebd9] uppercase tracking-wider truncate hover:underline hover:text-(--ac-gold-tx) transition-colors">
                          {member.name}
                        </h4>
                        <p className="text-[10px] text-(--ac-bronze-tx) font-serif font-bold uppercase tracking-wider truncate">
                          {member.title}
                        </p>
                        <p className="text-[9px] text-(--tx-mute) dark:text-[#a89379] font-serif truncate">
                          {member.parish}
                        </p>
                      </div>
                    </div>
